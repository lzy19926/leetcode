...
// 这里是自己封装的promise的js文件 

// ---------------------------封装Promise本体构造函数-------------------------------------------
    // 引入自己的js后可以用这里的promise替换掉全局中的原生promise
    // 这里实际上就是Promise类的构造函数
    function Promise(executor){  // 参数为执行函数
...
//----------------pending情况-------------------------------
    ...
    Promise.prototype.catch = function(onRejected){
     ...
     // 注意: Promise.resolve/reject方法都属于promise函数的   而不属于promise实例对象 
     //所以直接挂载到Promise函数上  而不是其prototype上
    //---------------------------- 添加Promise.resolve方法----------------------------------
    ...
    //-----------------------------------封装.all方法-----------------------------------------
    //返回promise对象  参数是一个promise数组  里面的三个p都成功  返回成功/
    //返回的promise结果是三个成功对象结果组成的数组
    // 三个p有一个失败  返回失败promise   其结果是数 组中失败promise的结果
    ...
    //-----------------------------封装race方法----------------------------------------------
    // race方法   let result = Promise.race([p1,p2,p3])   
    // 里面的参数  谁先改变  就由谁决定返回的promise结果  (判断异步执行速度)
