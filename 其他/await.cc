...
namespace v8
{
    namespace internal
    {
        ...
        // AsyncFunctionBuiltinsAssembler 类中定义了 AsyncFunctionAwait 函数
        template <typename Descriptor>
        void AsyncFunctionBuiltinsAssembler::AsyncFunctionAwait(
            const bool is_predicted_as_caught)
        {
            // 定义局部变量 async_function_object，并读入参数列表中描述符对应位置上的 JSAsyncFunctionObject 对象指针
            auto async_function_object =
                Parameter<JSAsyncFunctionObject>(Descriptor::kAsyncFunctionObject);

            // 定义局部变量 value，并读入参数列表中描述符对应位置上的 Object 对象指针
            auto value = Parameter<Object>(Descriptor::kValue);

            // 定义局部变量 context，并读入参数列表中描述符对应位置上的 Context 上下文对象指针
            auto context = Parameter<Context>(Descriptor::kContext);

            // 加载并创建两个待 Resolve 和 Reject 的 shared function info 节点，作为被传递给 Await() 方法
            TNode<SharedFunctionInfo> on_resolve_sfi =
                AsyncFunctionAwaitResolveSharedFunConstant();
            TNode<SharedFunctionInfo> on_reject_sfi =
                AsyncFunctionAwaitRejectSharedFunConstant();

            // 加载 outer_promise 局部变量节点, 并更新其值，在运行时环境进行异步操作完成后再返回 outer_promise

            TNode<JSPromise> outer_promise = LoadObjectField<JSPromise>(
                async_function_object, JSAsyncFunct$objectOffset);

            Await(context, async_function_object, value, outer_promise, on_resolve_sfi,
                  on_reject_sfi, is_predicted_as_caught);

            // 返回 outer_promise 局部变量节点，确保在向 BytecodeGenerator 暂停之前避免加载 outer_promise。
